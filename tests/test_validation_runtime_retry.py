...
class ValidationRuntimeRetryTest(unittest.TestCase):
    def setUp(self):
        ...
        self.node={'enabled':True,'labels':{'gpu_runtime_recovery':{'verified_at':20,'boot_id':'b','cuda_verified':True}},'gpus':[{'uuid':'g','enabled':True}],'policy':{'stable_polls':2}}
        self.snap={'boot_id':'b','received_at':25,'stable_polls':2,'read_ok':True,'d_state':0,'gpus':[{'uuid':'g'}]}
    ...
    def test_incomplete_recovery_is_rejected(self):
        for key,value in [('stable_polls',1),('gpus',[]),('received_at',-100),('boot_id','other'),('d_state',1)]:
